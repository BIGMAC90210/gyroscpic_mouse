...
double averageYOffset=0;
double averageZOffset=0;

const int totalReadingsCount=100;
double lastX[totalReadingsCount];
double lastY[totalReadingsCount];
unsigned long lastMillis[totalReadingsCount];
double totalMouseX=0;
double totalMouseY=0;

const int dwellRadius=20;
...
void loop()
{
  ...
  if(outputOn){
    ...
    if(averageCount<30){
      ...
      averageXOffset=averageXOffset/30;
      averageYOffset=averageYOffset/30;
      averageZOffset=averageZOffset/30;
      averageCount=100;
    } else {
...
void printGyro()
{
  ...
  Serial.print((dof.calcGyro(dof.gx)-averageXOffset), 2);
  Serial.print(", ");
  Serial.print((dof.calcGyro(dof.gy)-averageYOffset), 2);
  Serial.print(", ");
  Serial.print(dof.calcGyro(dof.gz)-averageZOffset, 2);
  Serial.print ("X= ");
  Serial.println((dof.calcGyro(dof.gx)-averageXOffset)*dof.calcAccel(dof.ax) +(dof.calcGyro(dof.gy)-averageYOffset)*dof.calcAccel(dof.ay) , 2);
...
// wait until there is both an accelerometer and gyroscope reading
  ...
  double mouseX=(averageXOffset-dof.calcGyro(dof.gx))*dof.calcAccel(dof.ax) +(averageYOffset-dof.calcGyro(dof.gy))*dof.calcAccel(dof.ay);
  double mouseY=averageZOffset-dof.calcGyro(dof.gz);
  Mouse.move(mouseX,mouseY );
  
  addLastPosition(mouseX,mouseY);
  double totalMovement=  sqrt(totalMouseX*totalMouseX + totalMouseY*totalMouseY);
  if(totalMovement > dwellRadius){
      resetLastMousePositions();
  } else {
    if(lastResetTime<(millis()-dwellTime)){
      Mouse.click();
      resetLastMousePositions();
      // reduce waiting time
    lastResetTime=millis()+dwellTime-dwellRepeatTime;
    }
  }
...
void recordAverageGyro(){
  // add another gyroscope reading to build up an average.
    dof.readGyro();
averageXOffset+=dof.calcGyro(dof.gx);
averageYOffset+=dof.calcGyro(dof.gy);
averageZOffset+=dof.calcGyro(dof.gz);
averageCount=averageCount+1;
}

void addLastPosition(double x, double y){
  
  unsigned long DwellAgo=max(millis()-dwellTime,0);
  totalMouseX=x;
  totalMouseY=y;
  
  for (int i=1;i<totalReadingsCount;i++){
     if(lastMillis[i]>DwellAgo){
       lastX[i-1]=lastX[i];
       lastY[i-1]=lastY[i];
       lastMillis[i-1]=lastMillis[i];
       totalMouseX=totalMouseX+lastX[i-1];
       totalMouseY=totalMouseY+lastY[i-1];
     } else {
       lastX[i-1]=-100;
       lastY[i-1]=-100;
       lastMillis[i-1]=0;
  ...
  lastX[totalReadingsCount-1]=x;
  lastY[totalReadingsCount-1]=y;  
  lastMillis[totalReadingsCount-1]=millis();
}

void resetLastMousePositions(){
